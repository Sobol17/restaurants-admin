...
const copyClientProfileLink = restaurantId => {
  const resolved = router.resolve({ name: 'profile', query: { restaurantId } })
  ...
  <div>
    <div class="card">
      ...
      >
        ...
        >
          <template #body="slotProps">
            {{ formatBoolean(slotProps.data.sbpIsEnabled) }}
          </template>
        </Column>
        <Column
          field="splitIsEnabled"
          header="Сплит"
          sortable
          style="min-width: 8rem"
        >
          <template #body="slotProps">
            {{ formatBoolean(slotProps.data.splitIsEnabled) }}
          </template>
        </Column>
        <Column
          field="phonePaymentIsEnabled"
          header="Оплата по телефону"
          sortable
          style="min-width: 14rem"
        >
          <template #body="slotProps">
            {{ formatBoolean(slotProps.data.phonePaymentIsEnabled) }}
          </template>
        </Column>
        <Column
          field="cashIsEnabled"
        ...
        >
          <template #body="slotProps">
            {{ formatBoolean(slotProps.data.cashIsEnabled) }}
          </template>
        </Column>
        <Column
        ...
        <Column field="isBanned" header="Бан" sortable style="min-width: 8rem">
          <template #body="slotProps">
        ...
        <Column :exportable="false" style="min-width: 10rem">
          <template #body="slotProps">
            <a
              href="#"
              class="mr-2 inline-flex items-center justify-center w-8 h-8 rounded-full border border-surface-300 text-surface-600 hover:bg-surface-100 transition-colors"
              title="Копировать ссылку на клиентскую часть"
              @click.prevent="copyClientProfileLink(slotProps.data.id)"
            >
          ...
          <div class="p-4">
            <div class="flex flex-col gap-6">
              ...
              <div>
                <h5>Часы работы</h5>
                <div
                  v-if="getWorkingHoursList(slotProps.data.workingHours).length"
                  class="mt-2 grid gap-2 text-sm"
                >
                  <div
                    v-for="(item, index) in getWorkingHoursList(
                      slotProps.data.workingHours,
                    )"
    ...
    >
      <div class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label for="owner-id" class="block font-bold mb-3"
              >ID владельца</label
            >
            <AutoComplete
              id="owner-id"
          ...
          <div>
            <label for="restaurant-name" class="block font-bold mb-3"
              >Название</label
            >
            <InputText
              id="restaurant-name"
            ...
            />
            <small
              v-if="submitted && !restaurantState.name"
              class="text-red-500"
              >Обязательное поле</small
            >
          </div>
          <div class="md:col-span-2">
            <label for="restaurant-address" class="block font-bold mb-3"
              >Адрес</label
            >
            <InputText
            ...
            />
            <small
              v-if="submitted && !restaurantState.address"
              class="text-red-500"
              >Обязательное поле</small
            >
          </div>
          <div>
            <label for="min-order" class="block font-bold mb-3"
              >Мин. сумма заказа</label
            >
            <InputText
              id="min-order"
              v-model.trim="restaurantState.minOrderSum"
              type="number"
              fluid
            />
          </div>
          <div>
            <label for="free-delivery" class="block font-bold mb-3"
              >Бесплатная доставка от</label
            >
            <InputText
              id="free-delivery"
              v-model.trim="restaurantState.freeDeliveryFrom"
              type="number"
              fluid
            />
          </div>
          <div>
            <label for="loyalty-percent" class="block font-bold mb-3"
              >Процент лояльности</label
            >
            <InputText
              id="loyalty-percent"
              v-model.trim="restaurantState.loyaltyPercent"
              type="number"
              fluid
            />
          </div>
          <div>
            ...
            <InputText
              id="latitude"
              v-model.trim="restaurantState.latitude"
              type="number"
              fluid
            />
          </div>
          <div>
            ...
            <InputText
              id="longitude"
              v-model.trim="restaurantState.longitude"
              type="number"
              fluid
            />
          </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.loyaltyIsEnabled"
                inputId="loyalty-enabled"
                binary
              />
            ...
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.cardIsEnabled"
                inputId="card-enabled"
                binary
              />
            ...
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.promoIsEnabled"
                inputId="promo-enabled"
                binary
              />
            ...
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.sbpIsEnabled"
                inputId="sbp-enabled"
                binary
              />
              <label for="sbp-enabled">СБП включён</label>
            </div>
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.splitIsEnabled"
                inputId="split-enabled"
                binary
              />
              <label for="split-enabled">Сплит включён</label>
            </div>
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.phonePaymentIsEnabled"
                inputId="phone-payment-enabled"
                binary
              />
              <label for="phone-payment-enabled">Оплата по телефону</label>
            </div>
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.cashIsEnabled"
                inputId="cash-enabled"
                binary
              />
              <label for="cash-enabled">Наличные</label>
            </div>
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.isPaused"
                inputId="is-paused"
                binary
              />
            ...
            <div class="flex items-center gap-2">
              <Checkbox
                v-model="restaurantState.isBanned"
                inputId="is-banned"
                binary
              />
        ...
        <div>
          <label for="payout-info" class="block font-bold mb-3"
            >Платежная информация</label
          >
          <Textarea
            id="payout-info"
            v-model.trim="restaurantState.payoutInfo"
            rows="3"
            fluid
          />
        </div>
        ...
        <div>
          ...
          <div class="flex flex-col gap-4">
            ...
            >
              ...
              <div
                v-if="workingHoursDays[day.key].length"
